...
from .flownet2 import FlowNetC, FlowNetFusion, FlowNetS, FlowNetSD
from .resample2d_package.resample2d import Resample2d
from .submodules import tofp16, tofp32
...
class FlowNet2(nn.Module):
    def __init__(
        ...
        batchNorm=False,
        div_flow=20.0,
        return_feat_maps: bool = False,  # pylint: disable=unused-argument
    ):
        super().__init__()
        self.batchNorm = batchNorm
        self.div_flow = div_flow
        self.return_feat_maps = False
    ...
    def init_deconv_bilinear(self, weight):  # pylint: disable=no-self-use
        f_shape = weight.size()
...
class FlowNet2C(FlowNetC.FlowNetC):
    def __init__(self, args, batchNorm=False, div_flow=20):
        super().__init__(  # pylint: disable=redundant-keyword-arg
            args, batchNorm=batchNorm, div_flow=20
        )
        self.rgb_max = args.rgb_max
...
class FlowNet2S(FlowNetS.FlowNetS):
    def __init__(self, args, batchNorm=False, div_flow=20):
        super().__init__(  # pylint: disable=redundant-keyword-arg
            args, input_channels=6, batchNorm=batchNorm
        )
        self.rgb_max = args.rgb_max
        self.div_flow = div_flow
...
class FlowNet2CS(nn.Module):
    def __init__(self, args, batchNorm=False, div_flow=20.0):
        ...
        # First Block (FlowNetC)
        self.flownetc = FlowNetC.FlowNetC(  # pylint: disable=redundant-keyword-arg
        ...
        # Block (FlowNetS1)
        self.flownets_1 = FlowNetS.FlowNetS(  # pylint: disable=redundant-keyword-arg
            args, batchNorm=self.batchNorm
        )
        self.upsample2 = nn.Upsample(scale_factor=4, mode="bilinear")
...
class FlowNet2CSS(nn.Module):
    def __init__(self, args, batchNorm=False, div_flow=20.0):
        ...
        # Block (FlowNetS2)
        self.flownets_2 = FlowNetS.FlowNetS(  # pylint: disable=redundant-keyword-arg
            args, batchNorm=self.batchNorm
        )
        self.upsample3 = nn.Upsample(scale_factor=4, mode="nearest")
