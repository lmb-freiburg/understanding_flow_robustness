...
class FlowNet2S(nn.Module):
    def __init__(self, input_channels=6, batchNorm=False, return_feat_maps=False):
        super().__init__()
    ...
    def normalize_correctly(self, im):  # pylint: disable=no-self-use
        mean = np.array([0.4114511, 0.43205959, 0.45015125])  # RGB
    ...
    def forward(self, x1, x2):
        ...
        if self.training:
            ...
            if self.return_feat_maps:
                return self.upsample1(flow2 * 20), []
            else:
                return self.upsample1(flow2 * 20)
