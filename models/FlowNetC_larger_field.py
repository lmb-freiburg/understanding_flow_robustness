...
class FlowNetC_larger_field(nn.Module):
    expansion = 1

    def __init__(
        self, batchNorm: bool = False, div_flow: int = 1, return_feat_maps: bool = False
    ):
        super().__init__()
        ...
        self.conv11 = conv(self.batchNorm, 64, 64, kernel_size=5, stride=1)
        self.conv2 = conv(self.batchNorm, 64, 128, kernel_size=5, stride=2)
        self.conv21 = conv(self.batchNorm, 128, 128, kernel_size=5, stride=1)
        self.conv3 = conv(self.batchNorm, 128, 256, kernel_size=5, stride=2)
        self.conv31 = conv(self.batchNorm, 256, 256, kernel_size=5, stride=1)
    ...
    def forward(self, x1, x2):
        ...
        out_conv1a = self.conv11(self.conv1(x1))
        if self.return_feat_maps:
            return_feat_maps.append(out_conv1a.clone())
        out_conv2a = self.conv21(self.conv2(out_conv1a))
        if self.return_feat_maps:
            return_feat_maps.append(out_conv2a.clone())
        out_conv3a = self.conv31(self.conv3(out_conv2a))
        if self.return_feat_maps:
            return_feat_maps.append(out_conv3a.clone())

        out_conv1b = self.conv11(self.conv1(x2))
        if self.return_feat_maps:
            return_feat_maps.append(out_conv1b.clone())
        out_conv2b = self.conv21(self.conv2(out_conv1b))
        if self.return_feat_maps:
            return_feat_maps.append(out_conv2b.clone())
        out_conv3b = self.conv31(self.conv3(out_conv2b))
        if self.return_feat_maps:
