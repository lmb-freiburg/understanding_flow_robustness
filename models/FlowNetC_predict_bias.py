...
class FlowNetC_predict_bias(nn.Module):
    expansion = 1

    def __init__(self, batchNorm=False, div_flow=1, return_feat_maps=False):
        super().__init__()
...
def flownetc_predict_bias(data=None, div_flow=1, finetune=False):
    """FlowNetC model architecture from the
    "Learning Optical Flow with Convolutional Networks" paper (https://arxiv.org/abs/1504.06852)

    Args:
        data : pretrained weights of the network. will create a new one if not set
    """
    model = FlowNetC_predict_bias(batchNorm=False, div_flow=div_flow)
    if data is not None:
        model.load_state_dict(
            data["model_state_dict"], strict=False if finetune else True
        )
    return model
