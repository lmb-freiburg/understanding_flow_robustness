...
class Preprocess(torch.nn.Module):
    ...
    def forward(self, variableInput):
        ...
        if self.pre_normalization is not None:
            if hasattr(self.pre_normalization, "mean") and hasattr(
                self.pre_normalization, "std"
            ):
        ...
        for intConv in range(5):
            try:
                self.moduleBasic[intConv * 2].weight.data.copy_(
                    torch.load(
                        "pretrained_models/spynet_models/modelL"
                        + str(intLevel + 1)
                        + "_"
                        + arguments_strModel
                        + "-"
...
class Backward(torch.nn.Module):
    ...
    def forward(self, variableInput, variableFlow):
        ...
        if (
            hasattr(self, "tensorGrid") == False  # pylint: disable=singleton-comparison
            or self.tensorGrid.size(0) != variableInput.size(0)
            or self.tensorGrid.size(2) != variableInput.size(2)
            or self.tensorGrid.size(3) != variableInput.size(3)
        ):
            torchHorizontal = (
                torch.linspace(-1.0, 1.0, variableInput.size(3))
            ...
            torchVertical = (
                torch.linspace(-1.0, 1.0, variableInput.size(2))
            ...
            self.tensorGrid = torch.cat(  # pylint: disable=attribute-defined-outside-init
                [torchHorizontal, torchVertical], 1
            ).cuda()
        # end
        # pylint: enable=access-member-before-definition

        variableFlow = torch.cat(
            [
                variableFlow[:, 0:1, :, :] / ((variableInput.size(3) - 1.0) / 2.0),
                variableFlow[:, 1:2, :, :] / ((variableInput.size(2) - 1.0) / 2.0),
            ],
            1,
        )
        ...
        return torch.nn.functional.grid_sample(
            input=variableInput,
            grid=variableGrid.clamp(-1.0, 1.0).permute(0, 2, 3, 1),
            mode="bilinear",
        )
...
class Network(torch.nn.Module):
    ...
    def __init__(self, nlevels, strmodel="F", pre_normalization=None, pretrained=True):
        ...
        self.moduleBasic = torch.nn.ModuleList(
            [Basic(intLevel, strmodel) for intLevel in range(nlevels)]
        )
    ...
    def forward(self, variableFirst, variableSecond):
        ...
        for intLevel in range(self.nlevels - 1):
            ...
            variableFirst.insert(
                0,
                torch.nn.functional.avg_pool2d(
                    input=variableFirst[0], kernel_size=2, stride=2
                ),
            )
            variableSecond.insert(
                0,
                torch.nn.functional.avg_pool2d(
                    input=variableSecond[0], kernel_size=2, stride=2
                ),
            )
            # end
        # end
        ...
        variableFlow = torch.autograd.Variable(
            data=torch.zeros(
                variableFirst[0].size(0),
                2,
                int(math.floor(variableFirst[0].size(2) / 2.0)),
                int(math.floor(variableFirst[0].size(3) / 2.0)),
            ).cuda()
        )

        for intLevel, _ in enumerate(variableFirst):
            variableUpsampled = (
                torch.nn.functional.upsample(
                    input=variableFlow, scale_factor=2, mode="bilinear"
                )
                * 2.0
            )

            if variableUpsampled.size(2) != variableFirst[intLevel].size(2):
                variableUpsampled = torch.nn.functional.pad(
                    variableUpsampled, [0, 0, 0, 1], "replicate"
                )
            if variableUpsampled.size(3) != variableFirst[intLevel].size(3):
                variableUpsampled = torch.nn.functional.pad(
                    variableUpsampled, [0, 1, 0, 0], "replicate"
                )

            variableFlow = (
                self.moduleBasic[intLevel](
                    torch.cat(
                        [
                            variableFirst[intLevel],
                            self.moduleBackward(
                                variableSecond[intLevel], variableUpsampled
                            ),
                            variableUpsampled,
                        ],
                        1,
