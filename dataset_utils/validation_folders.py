...
def crawl_folders(folders_list):
    ...
    for folder in folders_list:
        ...
        for img in current_imgs:
            ...
            assert d.isfile(), f"depth file {str(d)} not found"
            depth.append(d)
...
def crawl_folders_pair(folders_list):
    ...
    for folder in folders_list:
        ...
        current_depth = []
        for (img1, _) in zip(current_imgs1, current_imgs2):
...
class ValidationSet(data.Dataset):
    ...
    def __init__(self, root, transform=None):
        ...
        self.scenes = [
            self.root / folder[:-1]
            for folder in open(  # pylint: disable=consider-using-with
                scene_list_path, encoding="uft-8"
            )
        ]
...
class ValidationSetPair(data.Dataset):
    ...
    def __init__(self, root, transform=None):
        ...
        self.scenes = [
            self.root / folder[:-1]
            for folder in open(  # pylint: disable=consider-using-with
                scene_list_path, encoding="utf-8"
            )
        ]
