...
from dataset_utils.augmentor import FlowAugmentor, SparseFlowAugmentor


class FlowDataset(data.Dataset):
    def __init__(self, aug_params=None, sparse=False, transforms=None):
        self.augmentor = None
        self.sparse = sparse
    ...
    def __getitem__(self, index):
        ...
        if self.is_test:
            img1 = data_utils.read_gen(self.image_list[index][0])
            img2 = data_utils.read_gen(self.image_list[index][1])
        ...
        valid = None
        if self.sparse:
            flow, valid = data_utils.readFlowKITTI(self.flow_list[index])
        else:
            flow = data_utils.read_gen(self.flow_list[index])
...
class FlyingChairs(FlowDataset):
    def __init__(
        ...
        split_list = np.loadtxt("dataset_utils/chairs_split.txt", dtype=np.int32)
...
class FlyingThings3D(FlowDataset):
    def __init__(
        ...
        super().__init__(aug_params, transforms=transforms)
