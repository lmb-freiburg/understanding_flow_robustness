...
def load_oxts_packets_and_poses(oxts_files, origin_idx):
    ...
    for filename in oxts_files:
        with open(filename) as f:  # pylint: disable=unspecified-encoding
            for line in f.readlines():
                line = line.split()
                ...
                if len(oxts) >= origin_idx:
                    if origin is None:
                        ...
                        for o, _ in enumerate(oxts):
                            oxts[o] = oxts[o]._replace(
                                T_w_imu=origin.dot(oxts[o].T_w_imu)
                            )
...
class raw:
    ...
    def _get_file_lists(self):
        ...
        self.oxts_files = sorted(
            glob.glob(os.path.join(self.data_path, "oxts", "data", "*.txt"))
        )
        self.cam0_files = sorted(
            glob.glob(
                os.path.join(self.data_path, "image_00", "data", f"*.{self.imtype}")
            )
        )
        self.cam1_files = sorted(
            glob.glob(
                os.path.join(self.data_path, "image_01", "data", f"*.{self.imtype}")
            )
        )
        self.cam2_files = sorted(
            glob.glob(
                os.path.join(self.data_path, "image_02", "data", f"*.{self.imtype}")
            )
        )
        self.cam3_files = sorted(
            glob.glob(
                os.path.join(self.data_path, "image_03", "data", f"*.{self.imtype}")
            )
        )
        self.velo_files = sorted(
            glob.glob(os.path.join(self.data_path, "velodyne_points", "data", "*.bin"))
        )

        # Subselect the chosen range of frames, if any
        if self.frames is not None:
            self.oxts_files = subselect_files(self.oxts_files, self.frames)
            self.cam0_files = subselect_files(self.cam0_files, self.frames)
            self.cam1_files = subselect_files(self.cam1_files, self.frames)
            self.cam2_files = subselect_files(self.cam2_files, self.frames)
            self.cam3_files = subselect_files(self.cam3_files, self.frames)
            self.velo_files = subselect_files(self.velo_files, self.frames)
    ...
    def _load_timestamps(self):
        ...
        self.timestamps = []
        with open(timestamp_file) as f:  # pylint: disable=unspecified-encoding
            for line in f.readlines():
