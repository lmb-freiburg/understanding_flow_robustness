...
def interp_gt_flow(mat, mask=None):
    ...
    GD1 = interpolate.griddata(
        (x1, y1), newarr, (xx, yy), method="linear", fill_value=1e8
    )
    return GD1
...
def read_png_file(flow_file):
    ...
    for i, _ in enumerate(flow_data):
...
def resample(img, sz):
    ...
    wd = (yy - y0) * (xx - x0)
    out_flow[:, :, 0] = (
        (Ia[:, :, 0] * wa + Ib[:, :, 0] * wb + Ic[:, :, 0] * wc + Id[:, :, 0] * wd)
        * out_width
        / in_width
    )
    out_flow[:, :, 1] = (
        (Ia[:, :, 1] * wa + Ib[:, :, 1] * wb + Ic[:, :, 1] * wc + Id[:, :, 1] * wd)
        * out_height
        / in_height
    )
