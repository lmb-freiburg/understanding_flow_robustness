...
def makecolorwheel():
    ...
    # RY
    colorwheel[0:RY, 0] = 1
    colorwheel[0:RY, 1] = np.arange(0, 1, 1.0 / RY)
    col += RY

    # YG
    colorwheel[col : col + YG, 0] = np.arange(1, 0, -1.0 / YG)
    colorwheel[col : col + YG, 1] = 1
    col += YG

    # GC
    colorwheel[col : col + GC, 1] = 1
    colorwheel[col : col + GC, 2] = np.arange(0, 1, 1.0 / GC)
    col += GC

    # CB
    colorwheel[col : col + CB, 1] = np.arange(1, 0, -1.0 / CB)
    colorwheel[col : col + CB, 2] = 1
    col += CB

    # BM
    colorwheel[col : col + BM, 2] = 1
    colorwheel[col : col + BM, 0] = np.arange(0, 1, 1.0 / BM)
    col += BM

    # MR
    colorwheel[col : col + MR, 2] = np.arange(1, 0, -1.0 / MR)
