...
def flow_read(filename, return_validity=False):
    ...
    with open(filename, "rb") as f:
        ...
        assert (
            check == TAG_FLOAT
        ), " flow_read:: Wrong tag in flow file (should be: {}, is: {}). Big-endian machine? ".format(
            TAG_FLOAT, check
        )
        ...
        ), f" flow_read:: Wrong input size (width = {width}, height = {height})."
...
def flow_write(filename, uv, v=None):
    ...
    if v is None:
        ...
        if uv_.shape[0] == 2:
            ...
            raise Exception("Wrong format for flow input")
    else:
        u = uv
